...
class Testsolver(BaseSolver):
    ...
    def test(self):
        ...
        for batch in self.data_loader:
            ...
            t0 = time.time()
            with torch.no_grad():
                prediction = self.model(lms_image, pan_image)
            ...
            # self.save_img(lms_image.cpu().data, name[0][0:-4]+'_ms.tif', mode='RGB') #
            self.save_img(bms_image.cpu().data, name[0][0:-4]+'_bic.png', mode='RGB') #

            self.save_img(ms_image.cpu().data, name[0][0:-4]+'_gt.png', mode='RGB')
            self.save_img(prediction.cpu().data, name[0][0:-4]+f'_{method}.png', mode='RGB')
            pan = pan_image.squeeze(0).permute(1,2,0).cpu().numpy()
            pan = (pan*255).astype(np.uint8)
            pan = cv2.convertScaleAbs(cv2.normalize(pan, None, 0, 255, cv2.NORM_MINMAX))
            save_dir = os.path.join(self.cfg['test']['save_dir'], self.cfg['test']['type'])
            cv2.imwrite( os.path.join(save_dir,name[0][0:-4]+'_pan.png'),pan[:,:][:,:,np.newaxis])
    ...
    def save_img(self, img, img_name, mode):
        save_img = img.squeeze().clamp(0, 1).permute(1,2,0).contiguous().numpy()
        #print((save_img.max()))
        # save img
        save_dir = os.path.join(self.cfg['test']['save_dir'], self.cfg['test']['type'])
        ...
        #print(save_img.max())
        save_img = Image.fromarray(save_img[:,:,:3], mode)
